...
```
Partition = 一本日记（append-only，有页码 offset）
Consumer = 拿书签的读者（自己决定从哪页开始读、读多快）
消息生命周期 = 永久保留（到期整段删除）
失败处理 = 不管（Consumer 自己负责）
```

→ 天然适合：事件溯源、流回放、大数据管道
→ 天然不适合：需要"投递保障语义"的业务场景（无事务/延迟/死信）

**RocketMQ = 快递公司**

```
Broker = 快递仓库（可延迟派送、可"暂存待确认"、可自动重试）
Consumer = 收件人（派送失败有标准处置流程）
消息生命周期 = 投递完成即结束
失败处理 = 自动重试 → 死信队列 → 人工介入
```

→ 天然适合：订单、支付、物流等业务流程驱动
→ 天然不适合：纯日志流/流处理（功能过重是浪费）

**RabbitMQ = 电话交换机**

```
Exchange = 交换机（根据 routing key 转接到对应 Queue）
Consumer = 等电话接进来的人（Broker Push，接通即消费）
消息生命周期 = 消费即删（无回溯）
失败处理 = Nack + 死信
```

→ 天然适合：IM、实时通知、多系统联动、灵活路由
→ 天然不适合：大量堆积（设计哲学是"消息应被立即消费"）
...
| 维度 | 排序 |
|------|------|
| 吞吐量 | Kafka(百万) >> RocketMQ(十万) >> RabbitMQ(万) |
| 延迟 | RabbitMQ(μs) < RocketMQ(ms) < Kafka(ms~百ms) |
| 堆积 | Kafka ≈ RocketMQ(TB级) >> RabbitMQ(百万级即断崖) |

### 选型陷阱

| 陷阱 | 本质 |
|------|------|
| "Kafka 万能" | 它是日志系统，不是业务消息中间件 |
| "RabbitMQ 够用" | 堆积是硬伤，量大必崩 |
