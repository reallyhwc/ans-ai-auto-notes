...
> 关联: [[./Redis 常用数据类型与使用场景.md]] — 跳表 vs B+树对比 | [[./MySQL 索引失效场景全解.md]] — B+ 树建好了却用不上：索引失效的典型场景
...
**不强相关。** `innodb_page_size` 是 MySQL 的软件配置，默认 16KB，在初始化时设定。可选 4KB/8KB/16KB/32KB/64KB，与磁盘是 HDD 还是 SSD 没有绑定关系。16KB 是一个工程甜点位——够大（一页存上千 key）又够小（I/O 延迟可控）。
...
```bash
# 只能在初始化时设置，之后不能改
mysqld --initialize --innodb-page-size=8192
```

### 跳表删除节点时，上层怎么处理？

**从所有出现该节点的层中直接移除。** 就像从 N 个有序链表中同时删掉同一个元素。

```
删除 5 前后对比:

Level 2:  [1] → [5] → [9]     →     [1] ────────→ [9]
Level 1:  [1]→[3]→[5]→[7]→[9]  →     [1]→[3]→[7]→[9]
Level 0:  [1]↔[3]↔[5]↔[7]↔[9]  →     [1]↔[3]↔[7]↔[9]
```

每一层找到前驱节点，改 forward 跳过被删节点。逻辑和普通链表删除一样，多做几层。Redis 源码中 `zslDeleteNode` 通过 `update[]` 数组（每层的前驱节点）一次遍历完成所有层的指针调整。

### B+ 树非叶节点存的 key 什么时候变动？

非叶节点存的不是"范围"，而是**分界 key**。它们只在页分裂或页合并时变动：

| 场景 | 非叶节点变动 |
|------|-------------|
| INSERT 触发页分裂 | 新的分界 key 插入父节点 |
| DELETE 触发页合并 | 对应的分界 key 从父节点移除 |
| 普通增删改（不触发分裂/合并） | **完全不改变** |

**与跳表的本质区别**：

```
跳表上层: 直接指向具体数据节点 → 数据变 → 上层指针必变
B+ 树非叶: 只指向子页 → 页内数据怎么变关上层什么事 → 页边界不变就不动
```

B+ 树非叶节点关注的是"页的边界"，不关心页里面具体有哪些数据。只要不触发分裂/合并，非叶节点纹丝不动。跳表相反——上层直接指向具体数据节点，数据动指针必动。

### 联合索引 (A, B, C, D) 的存储结构与最左前缀

#### 物理存储

联合索引是一棵 B+ 树，按 A → B → C → D 的多级排序，叶子存 `(A, B, C, D, 主键值)`。就像 Excel 的多级排序——先按 A 排，A 相同再按 B 排，依此类推。

```
叶子节点排序示例:

(A=10, B=3, C=5, D=1) → pk=101
(A=10, B=3, C=5, D=2) → pk=205    ← A=10 内部 B=3 内部按 C,D 排
(A=10, B=3, C=8, D=1) → pk=87
(A=10, B=5, C=1, D=9) → pk=312    ← A=10 内部 B 变了
(A=150, B=2, C=1, D=3) → pk=45    ← A 变了
(A=150, B=2, C=1, D=7) → pk=198
(A=150, B=7, C=3, D=1) → pk=76
```
...
WHERE B = 3   -- ❌ 不命中
WHERE C = 5   -- ❌ 不命中
WHERE A = 10 AND C = 5  -- ⚠️ 只用 A，C 跳过了 B → 失效
WHERE A > 10 AND B = 3  -- ⚠️ A 范围扫描，B 只用于排序不过滤
```

**原因**：B+ 树按 A→B→C→D 排序，B 在全局不是有序的——B=3 可能分散在 A=10、A=150、A=200 各个区域中。不给定 A，B+ 树无法定位起点 → 只能全扫。
...
flowchart TD
    ...
    F1 -->|"是"| F2{"A 是 = 吗？"}
    F2 -->|"是 ="| F3{"包含 B？"}
    F2 -->|"范围查询"| P1["⚠️ 只用 A<br/>后续列只能排序"]
    F3 -->|"是"| F4{"B 是 = 吗？"}
    F3 -->|"否（跳过）"| P2["✅ 只用 A<br/>跳过的列之后全失效"]
    F4 -->|"是 ="| F5["继续 C → D<br/>同上逻辑"]
    F4 -->|"范围查询"| P3["⚠️ 只用 A,B<br/>C,D 只能排序"]
```

**口诀**：`=` 可以乱序（优化器自动调），第一个范围查询之后全失效，跳过的列之后全失效。
