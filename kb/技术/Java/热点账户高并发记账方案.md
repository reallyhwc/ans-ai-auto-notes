...
> 关联: [分布式事务](./分布式事务全景.md) — 热点账户方案常与分布式事务结合使用 | [[./JVM 内存模型与垃圾回收.md]] — 高并发场景下大量短生命周期对象，GC 调优直接相关
> 关联: [CAP 理论与工程选型](<../计算机基础/CAP 理论与工程选型.md>) — 缓冲记账/子账户拆分的取舍本质是 CAP 权衡
...
CREATE TABLE account_buffer (
    ...
    status TINYINT DEFAULT 0,            -- 0:待处理 1:已抢占(处理中) 2:已入账 3:异常
