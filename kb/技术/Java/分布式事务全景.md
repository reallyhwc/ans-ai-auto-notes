...
> 关联: [RocketMQ 底层实现原理](<./RocketMQ 底层实现原理.md>) — 事务消息是分布式事务方案之一 | [Dubbo 与 RPC 框架横评](<./Dubbo 与 RPC 框架横评.md>) — 分布式服务调用的 RPC 层（2PC/TCC/Saga 的执行载体）
> 关联: [[./InnoDB 如何实现 ACID.md]] — 单机 ACID 是分布式事务的地基 | [[./热点账户高并发记账方案.md]] — 本地消息表 + 对账兜底的工程落地实例 | [[../计算机基础/CAP 理论与工程选型.md]] — 分布式事务两大阵营本质是 CAP 取舍
...
### 12.6 XA Phase 2 的不一致窗口

#### XA 不是"拆成多个小事务"

CN 不会把一条分布式事务"拆解"为多个独立小事务。实际模型是：**每个 DN 运行一个本地 XA 事务（可含多条 SQL），CN 只负责协调它们一起 PREPARE + COMMIT**。

```mermaid
graph LR
    subgraph "DN1（同一 XA 事务）"
        A1["UPDATE account"]
        A2["INSERT flow"]
    end
    subgraph "DN2（另一 XA 事务）"
        B1["UPDATE trade"]
    end
    CN["CN 协调器"] -->|"XA PREPARE"| A1
    CN -->|"XA PREPARE"| B1
    CN -->|"XA COMMIT"| A1
    CN -->|"XA COMMIT"| B1
```
...
InnoDB RR 级别下，MVCC（快照读）+ Gap Lock（当前读）解决了单 DN 内的幻读问题。但 MVCC 是 MySQL 实例级别机制，**跨 DN 的快照是各自独立的**——DN_A 的 MVCC 快照和 DN_B 的 MVCC 快照互不感知。

#### Phase 2 不一致窗口——2PC 的本质局限

```mermaid
sequenceDiagram
    participant CN as CN 协调器
    participant A as DN_A
    participant B as DN_B
    participant C as DN_C
    participant Other as 其他事务

    Note over CN,C: Phase 1: PREPARE（全部持锁，不可见）
    CN->>A: XA PREPARE → YES
    CN->>B: XA PREPARE → YES
    CN->>C: XA PREPARE → YES

    Note over CN,C: Phase 2: COMMIT（不一致窗口出现）
    CN->>A: XA COMMIT → ✅ 释放锁
    Note over Other: ⚠️ 能看到 A 的变更

    CN->>B: XA COMMIT → ✅ 释放锁
    Note over Other: ⚠️ 能看到 A+B 的变更

    CN->>C: XA COMMIT → ❌ 网络超时
    Note over Other,C: ❌ 不一致窗口：A+B 可见，C 不可见
    ...
    alt C 最终恢复
        C-->>CN: COMMIT 成功
        Note over Other: 不一致窗口关闭 ✅
    else C 永久损坏
        Note over CN,C: 永久不一致 → 人工补偿 ❌
    end
```

| 时间点 | A | B | C | 其他事务可见性 |
|--------|---|---|---|--------------|
| PREPARE 完成 | 持锁 | 持锁 | 持锁 | 全部不可见 ✅ |
| A COMMIT 后 | 释放锁 | 持锁 | 持锁 | 只看到 A ⚠️ |
| AB COMMIT 后 | 释放锁 | 释放锁 | 持锁 | 看到 A+B ⚠️ |
| C COMMIT 失败 | 释放锁 | 释放锁 | 持锁/重试 | A+B 可见，C 不可见 ❌ |

**这不是脏读**——AB 已 commit，读到的是已提交数据。问题是分布式事务的原子性在窗口内被打破了。

#### 窗口大小评估

- **正常情况**：Phase 2 全部 COMMIT 耗时 < 10ms，窗口几乎不存在
- **异常情况**：某个 DN 网络超时，窗口 = 重试间隔（秒级）
- **极端情况**：DN 永久宕机 + 磁盘损坏 → 永久不一致（需人工介入补偿）

#### 工程选择

| 场景 | 对不一致窗口的容忍度 | 方案选择 |
|------|-------------------|---------|
| DRDS 跨分片 SQL | 窗口极短可接受 | 内置 XA |
| 金融转账 | 窗口可控 + 有人工兜底 | XA + 对账补偿 |
| 电商下单（高并发） | 不可接受 | TCC / 事务消息（最终一致） |
