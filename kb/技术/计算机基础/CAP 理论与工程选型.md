...
代表系统：**单机/单实例 RDBMS（一台 MySQL 实例、Oracle 单实例），或"整个机房当一个整体"时的小集群**
...
> ⚠️ 口径澄清：**"MySQL 主从"不能算 CA**。主从本身就是两个节点，主从之间会出现网络分区/复制延迟，按 §4.2 的判定它是 **AP（异步复制）或 CP（半同步复制）**。这里的 CA 只描述"单个数据库实例"或"把同机房集群视为一个不可分割整体"时的定位——只要节点之间存在可能被切断的通信链路，P 就必须纳入考量。
