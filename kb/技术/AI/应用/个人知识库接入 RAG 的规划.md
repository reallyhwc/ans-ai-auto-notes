...
> 最后整理: 2026-09-11 | 来源: 对话分析 + 项目现状评估
...
| 总行数 | **32,966 行** |
| 总字符数 | **~1.4MB** |
| 最大文件 | 1,222 行（Redis 篇） |
| 平均每文件 | ~412 行 |

> 2026-05-19 首次评估时是 34 篇 / 9,641 行 / ~383KB；四个月内容量翻倍以上，**§2 的两条阈值（>80 篇、>500KB）现已同时触发**。

**结论（2026-09-11 复核）：阈值已触发，RAG 进入「待启动」状态。** 1.4MB 文本已无法可靠地全量塞进单次 context（约 400-500KB 中文为上限），`codebase_search` + overview.html 全文搜索在 80 篇规模下开始出现"写过但搜不准"。本文档保持"规划"定位——**尚未实施**，启动时机由用户决定（见 §6 清单）。
...
1. **全量可读性已跌破**（2026-09-11 更新）— 1.4MB 已超过单次 context 能可靠容纳的范围，§5 原有的"全量可读"前提不再成立
2. **投入产出不匹配** — 建设成本 > 当前收益，不如继续积累内容
3. **搜索在 80 篇规模下开始吃力** — `codebase_search` + overview.html 全文搜索仍可用，但召回质量下降（本地 BM25 索引是成本最低的第一步）
...
当以下条件满足 **任意两项** 时，启动第一步（本地 BM25 索引）：

- [x] 知识库文件数 > 50 篇 —— **已满足（80 篇）**
- [x] 总字符数 > 400KB —— **已满足（~1.4MB）**
- [ ] 出现 2 次以上"我写过但搜不到"的情况
- [ ] overview.html 搜索体验明显不足

当以下条件满足 **任意两项** 时，启动第二步（MCP Server 暴露给 Agent）：

- [x] 知识库文件数 > 80 篇 —— **已满足（80 篇）**
