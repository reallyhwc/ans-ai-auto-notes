...
> 关联: [agent-development-practice](./agent-development-practice.md) — Agent 开发范式、框架选型、vs Java 对比（开发视角）
> 关联: [agent-patterns](./agent-patterns.md) — 四大范式深度展开（架构图 / Prompt 模板 / 典型案例）
> 关联: [llm-app-design](./llm-app-design.md) — 确定性 vs 概率性、幻觉防控
> 关联: [openai-agents-sdk](./openai-agents-sdk.md) — Multi-Agent 协作与 Handoff 机制

---

## 1. 相比传统 Java 应用，额外需要关心的 8 个维度

[agent-development-practice](./agent-development-practice.md) 已经从开发视角对比了 6 个维度（核心逻辑、输入输出、流程控制、测试、调试、核心技能）。下面从**运维/SRE 视角**补上开发时不容易看到的坑——这些都是实际跑起来之后才会暴露的问题。
...
> 关联: [agent-development-practice](./agent-development-practice.md) — 回到开发视角的范式选型与框架速查
> 关联: [agent-patterns](./agent-patterns.md) — 四大范式的深度架构展开
> 关联: [llm-app-design](./llm-app-design.md) — 确定性 vs 概率性应用的设计原则
