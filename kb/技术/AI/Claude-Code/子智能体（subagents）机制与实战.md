...
什么时候该用 Sub-Agent 处理高噪声？一条经验法则：

> **输出超过 50 行，且你只关心其中不到 1/5 → 用子代理。**

量化对比（以 `npm test` 300 行输出为例）：

| 方式 | 主对话 context 变化 | 后续每轮代价 |
|------|-------------------|------------|
| 主对话直接跑 | 3,500 → 8,800 tokens（+151%） | 每轮多带 4,500 tokens 噪声 |
| Sub-Agent 隔离 | 3,500 → 3,700 tokens（+6%） | 噪声用完即释放，零后续代价 |

### 18.5 三层输出格式设计法

Sub-Agent 的输出是主对话的输入。格式设计直接决定下游效率。

```mermaid
graph LR
    L1["第一层<br/>一眼结论<br/>Status: FAIL"] --> L2["第二层<br/>定位问题<br/>哪个失败+为什么"]
    L2 --> L3["第三层<br/>修复方向<br/>（可选，有价值时才出现）"]
...
| 原则 | 好的做法 | 差的做法 |
|------|---------|---------|
| **结论先行** | 第一行就是 `PASS/FAIL` | 先列 30 行 pass 细节再说结果 |
| **可操作性** | `[Form.test.js:45] email 未绑定 → 加 setEmail()` | "Form 模块有些测试失败了" |
| **分层详略** | 全 pass→一行 / 少量 fail→展开 / 大量 fail→按类归组 | 无论什么情况都输出相同详度 |
| **为下游消费设计** | 主 agent 拿到能直接生成修复代码 | 主 agent 还要再去读文件才能理解 |
...
全部通过 → "✓ PASS (47/47)"（极简）
少量失败 → 展开每个 fail case
大量失败 → 按类别归组："DB connection (8), Auth token (3), Input validation (1)"
```

> **可迁移洞察**：这套三层输出格式设计法本质是思维方法论——结论先行、分层详略、为"下游消费者"设计——同样适用于写作、口语表达、会议发言。
...
| **分析步骤明确** | 给 sub-agent 一个分析框架（Quick Scan → Timeline → Correlation） | 让它随意发散 |

### 18.7 与本项目的关联

本项目的 `kb-auditor` 就是一个高噪声任务处理的实例——它需要读大量文件内容（高噪声），但只返回一个 `VERDICT: pass/minor/major` + 几句关键观察（低噪声结论）。其输出格式也遵循三层法：VERDICT（一眼结论）→ 4 维评分（定位问题）→ 行动建议（修复方向）。
