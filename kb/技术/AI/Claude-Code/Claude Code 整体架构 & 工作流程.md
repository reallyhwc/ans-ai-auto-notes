...
```
你用什么权限运行 Claude Code，决定了它有多大自由度（Shift+Tab 循环切换）：

┌──────────────────┬──────────────────────────────────┐
│ 模式              │ 无提示自动执行什么                │
├──────────────────┼──────────────────────────────────┤
│ Manual(`default`) │ 只有读操作                       │
│                  │ 🔒 最保守，敏感工作用             │
├──────────────────┼──────────────────────────────────┤
│ acceptEdits      │ 读 + 文件编辑 + 常见文件系统命令  │
│                  │ （mkdir/touch/mv/cp）✅ 改代码时用 │
├──────────────────┼──────────────────────────────────┤
│ plan             │ 只读 + 制定方案，改动等你批准     │
│                  │ 📋 适合调研/规划阶段              │
├──────────────────┼──────────────────────────────────┤
...
│                  │ 🤖 CI / 无人值守脚本              │
├──────────────────┼──────────────────────────────────┤
│ bypassPermissions│ 一切（含 protected paths）       │
│                  │ ☠️ 只该在容器/VM 里跑             │
└──────────────────┴──────────────────────────────────┘
```
...
- **`acceptEdits` 不是"所有操作都放行"**：它放行文件编辑和常见文件系统命令，网络访问、其他 Bash 命令仍要确认
- **"全部自动执行"反而是 `bypassPermissions`**，它是唯一能自动写 protected paths 的模式，官方标注只用于隔离容器/VM
- **`Manual` 的配置值是 `default`**（hook 和 SDK 集成里用这个值）；CLI 从 v2.1.200 起也接受 `manual` 别名
- 模式只是基线，之上还能叠 `permissions` 的 allow / ask / **deny** 规则——deny 在**任何**模式下都生效，包括 `bypassPermissions`

> 官方参考：[Choose a permission mode](https://code.claude.com/docs/en/permission-modes)
