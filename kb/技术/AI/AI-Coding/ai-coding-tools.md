...
> 关联: [claude-code-architecture](../Claude-Code/claude-code-architecture.md) — Claude Code 技术架构 | [llm-agent-mcp](../大模型/llm-agent-mcp.md) — Agent 与 MCP 基础
...
**为什么有效？** 不同厂商的模型在训练数据、架构、偏好上不同，犯同样错误的概率大幅降低。实测下来 CR 覆盖面更全。这和 [Harness Engineering](../Claude-Code/harness-engineering.md) 中双 LLM 交叉校验（串行校验模式）是同一个原理。
...
> 关联: [AI Coding 团队治理](./ai-coding-team-governance.md) — Pre-PR 机制、零排期重构完整案例
> 关联: [Harness Engineering](../Claude-Code/harness-engineering.md) — 双 LLM 交叉校验四种实现方式
