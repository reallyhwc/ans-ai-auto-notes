...
§11 测出 MoE 快 2.3×，这一章回答"为什么"。**全部结论都在 M5 Pro 48GB 上实测过，不是抄来的。**
...
|---|---|---|---|
| **decode（生成）** | 36.3 tok/s | **86.4 tok/s** | **2.4×** |
| **prefill（读取输入）** | 145 tok/s | 177 tok/s | 1.2× |
| 冷启动加载 | 3.52 s | 5.22 s | 更慢 |
| 常驻内存 | 19.1 GB | 22.1 GB | +3GB |
| **答案正确性** | ✅ 2/3 小时 | ✅ 2/3 小时 | 打平 |
| 步骤完整度 | 完整（387 tok） | 被 400 tok 上限截断 | 略啰嗦 |

> 单请求场景下 MoE 实测在 **58~86 tok/s** 区间波动；**"MoE 明显更快"这个结论是稳的**。
...
| 代价 | 说明 | 对本地单人用有影响吗 |
|------|------|-------------------|
| 内存占用更高 | 22.1 vs 19.1 GB | 48G 上无感 |
...
**实测印证了内存悬崖**：开 `OLLAMA_MAX_LOADED_MODELS=2` 后，系统仍只允许 37.4GB 给 GPU——

```
尝试常驻 27b(19.1) + 35b(22.1) = 41.2 GB  >  37.4 GB 上限
实测结果：常驻 1 个，22.1 GB     ← Ollama 自己算完放不下，只留一个
```
