---
title: "本地部署 LLM"
description: "Ollama安装使用+进阶玩法(API/Embedding/Modelfile/Web UI)、小模型推荐；§4 量化命名详解（Q4/Q5/Q6、_K_/S/M/L 含义与选档决策）；含 §8 Mac 微调可行性与成本（统一内存带宽对比、MPS 三个坑与芯片代际无关、云 GPU 租用 vs Mac 训练的时间账）、§9 Mac 上能玩的 14 个项目 + M4/M5 实测 tok/s 基准、§10 48GB 模型选型（Q4 内存公式与推荐清单）+ 闲置资源成本真相（内存昂贵、算力免费）与 keep_alive 调优、§11 本地跑通 Qwen3.6 27B/MoE 的真实记录（MLX 大幅超线性推算、两个硬编码/常驻坑）"
---
...
> 数据来源：Apple 官方技术规格页（各代 Pro/Max 档均为 3 档带宽配置）。**自己查时认准"Pro / Max 分档 + 带宽字段"**，不要把 Pro 的数字当 Max 用。
...
**M5 新增的 Neural Accelerators 值得单独说一句**：Apple 宣称 M5 Pro/Max 的 GPU 内神经加速器带来 **4× 于 M4 Pro/Max 的峰值 AI 算力**，且首次采用 **Fusion Architecture**（把两颗第三代 3nm 裸片用先进封装拼成一颗）。但要清醒：
...
**48GB 档 Apple Silicon 上的预期**（⚠️ 推算，非实测——按带宽比例缩放）：
...
## 11. 实战：本地跑通 Qwen3.6 27B（一台 48GB Apple Silicon Mac 上的真实记录）

§9 和 §10 讲的都是原理和别人的数据，这一章是**在一台 48GB 统一内存的 Apple Silicon Mac 上亲手跑出来的真实数字**——包括一路踩的坑。**记录的是可复现的规律，不是特定机型的性能承诺。**
...
启动日志里能直接看到后端和上下文默认值，**这两行是判断"到底有没有用上 GPU"的关键证据**（下面把机型名与显存容量抹成占位符，实际是你的机器名与可用显存）：

```
INFO source=types.go  msg="inference compute" library=Metal name="<你的芯片>" total="<可用显存>" available="<可用显存>"
INFO source=routes.go msg="vram-based default context" total_vram="<可用显存>" default_num_ctx=32768
```

> 两点观察：**① 后端必须显示 `library=Metal`**，显示 CPU 就说明没吃到 GPU；**② `default_num_ctx` 是 Ollama 按可用显存自动推的**，容量越小给得越保守。
>
> ⚠️ **坑 1：密钥目录硬编码，无法用环境变量改。** Ollama 启动时一定要在家目录建 `id_ed25519` 密钥对，**二进制里没有对应的环境变量**（可以从 `strings ./ollama | grep OLLAMA_` 确认没有 key 路径项）。所以即使你用 `OLLAMA_MODELS` 把权重挪走了，**家目录还是会被写**——启动前得确保该目录可创建。
...
|------|--------------------------|------------------------|------|
| 权重体积 | 18 GB | 22 GB | MoE 更大 |
| 常驻内存 | 约 19 GB | 约 22 GB | MoE 多占约 3 GB |
| **生成速度（decode）** | 基准 | **明显更快** | **实测约 2.3×** |
| 首 token（prefill） | 基准 | 接近 | 打平 |
| **冷启动加载** | 更快 | **更慢** | 体积越大越慢 |
| 回答质量 | 正确、通顺 | 正确、更简洁 | 同一档 |

> ⚠️ 表里不写绝对 tok/s：**绝对值和机型的内存带宽强相关，抄数字没意义**。要看你自己的数字，按 §11.5 的脚本跑一遍即可。

**两条结论，都跟预期不一样：**

1. **MLX 版比线性推算的快一倍多。** §9.3 按高档位机型数据折半推算 48GB 档，实测速度是那个推算值的两倍以上。说明 (a) 同代较小档位的实际表现好于线性缩放估计，(b) **MLX 引擎的效率确实兑现了**。→ **教训：带宽缩放只能估个量级，别拿它当结论。**
2. **MoE 的速度优势在 Mac 上兑现得非常彻底**：实测约 **2.3 倍**，而两者常驻内存只差约 3 GB。→ **48GB 档 Mac 上，MoE 是毫无争议的首选形态。**
...
```
加载 35b-a3b 后：  常驻 1 个
再请求 27b-mlx：   常驻 1 个，换成了 27b-mlx   ← 35b-a3b 已被卸载
```

**后果**：交替使用两个模型 = 每次都付一次冷启动（体积 20GB 级，约数秒从 SSD 重读）。

**这不是 bug，是保护**——默认同一时刻只服务一个模型，避免内存被挤爆。想要两个都常驻：

```bash
OLLAMA_MAX_LOADED_MODELS=2 ./ollama serve &   # 两个模型同时常驻，容量上要留神
```

### 11.4 系统侧资源实况（对应 §10 的理论）

跑完一轮后，用两个命令看系统侧状态（这才是重点，不要抄数字）：

```bash
vm_stat                       # 看 free / active / inactive / wired 分布
sysctl vm.swapusage           # 看 swap 用量 ← 最关键的一个指标
```
...
| 指标 | 健康信号 | 说明 |
|------|---------|------|
| `wired` | ≈ 模型权重体积，且不再增长 | 权重被 pin 在物理内存里，**没被换到磁盘** → 速度才稳定 |
| `swap used` | **= 0** | ⭐ 最关键：一旦有 swap，说明已经越过内存墙，速度会断崖式下跌 |
| `free` | 偏小也不必慌 | macOS 会随时回收 inactive 内存，**free 小 ≠ 卡** |

> **判据**：只要 `swap used = 0`，就说明当前模型+上下文规模还在舒适区；一旦开始 swap，就该降档量化、缩短上下文，或换更小的模型。
...
**在一台 48GB 统一内存的 Apple Silicon Mac 上，`ollama pull qwen3.6:35b-a3b` 是"体验最好 / 性价比最高"的单条命令**：58 tok/s 的生成速度已经接近"读起来不觉得卡"的档位，22.5GB 常驻还留了一半内存，冷启动 5 秒可以接受。

> 想要更快的追问响应，把 `OLLAMA_KEEP_ALIVE` 调长（避免 5 分钟后卸载再冷启动）；想要跑长上下文，注意 `default_num_ctx=32768` 是 Ollama 按显存自动定的，显式调大会显著吃 KV Cache。

---

## 12. MoE vs Dense：同一道题实测 + 四个为什么

§11 测出 MoE 快 2.3×，这一章回答"为什么"。**全部结论都经过本地实测验证，不是抄来的。**

### 12.1 同一道题的公平对比

同一道数学题（水池注水：甲管 6h、乙管 4h，同开 2h 后关甲，乙还需几小时），`temperature=0`、关思考链：

| | **DENSE `qwen3.6:27b-mlx`** | **MoE `qwen3.6:35b-a3b`** | 倍数 |
|---|---|---|---|
| **decode（生成）** | 基准 | **明显更快** | **实测约 2.4×** |
| **prefill（读取输入）** | 基准 | 略快 | 约 1.2× |
| 冷启动加载 | 更快 | 更慢 | 体积更大 |
| 常驻内存 | 约 19 GB | 约 22 GB | +3GB |
| **答案正确性** | ✅ 2/3 小时 | ✅ 2/3 小时 | 打平 |
| 步骤完整度 | 完整 | 更啰嗦（更易撞输出上限） | MoE 话更多 |

> 单请求场景下 MoE 的绝对值会有波动（首次加载后与缓存态不同）；**"MoE 明显更快"这个结论是稳的**。

### 12.2 结构差异：FFN 被拆成多个"专家"

Transformer 每层 = **注意力层 + FFN 前馈层**，FFN 占参数量大头（约 2/3）。

```mermaid
flowchart TD
    subgraph DENSE["DENSE（普通模型）"]
        T1[token] --> F1["一个巨大的 FFN<br/>全部参数都要读"]
    end
    subgraph MoE["MoE（混合专家）"]
        T2[token] --> R[路由器 Router<br/>门控 + top-k]
        R -->|选中 2 个| E1[专家1]
        R -->|选中 2 个| E3[专家3]
        R -.->|未选中 不计算| E2[专家2]
        R -.->|未选中 不计算| EN[专家64]
...
**`35B-A3B` 怎么读**：**总共 35B 参数，每个 token 只激活 3B**（A = Active）。

本机实测的真实配置（`ollama show qwen3.6:35b-a3b` 读出来的元信息，比常见假设极端得多）：

```
qwen35moe.expert_count                    = 256   ← 256 个专家！
qwen35moe.expert_used_count               = 8     ← 每个 token 只用 8 个（3%）
qwen35moe.expert_feed_forward_length      = 512
qwen35moe.expert_shared_feed_forward_length = 512 ← 还有一个"共享专家"人人必过
qwen35moe.block_count                     = 41
qwen35moe.embedding_length                = 2048
```

**稀疏度 ≈ 8/256 = 3%**。粗算每个 token 的读取预算（约 3.5~4B）：

```
注意力层（Q/K/V/O + KV）         ≈ 0.15 B
8 个专家 FFN：41 层 × 8 × 512 × 2048 × 3 ≈ 1.03 B
共享专家 FFN（人人必过）：41 层 × 512 × 2048 × 3 ≈ 0.13 B
embedding + lm_head（词表大）     ≈ 0.5 B
──────────────────────────────────────────
≈ 1.8 B（已相当接近 A3B 宣称的 3B 量级）
```

> 两个细节值得记：**① 专家数越多，稀疏度越极端**（256→8 比 64→8 省得多）；**② "共享专家"是人人必过的**，用来兜住通用知识，避免每个专家都重复学基础能力——它不参与稀疏节省，是纯增质量的部分。

### 12.3 ⭐ 关键澄清：MoE 不减少**计算量**，减少的是**读取量**

这是最容易被误解的一点：

```
35B-A3B：生成 1 个 token → 要读 3B 参数的权重（不是 35B）
DENSE 27B：生成 1 个 token → 要读 27B 参数的权重
```

### 12.4 为什么更快：生成阶段的瓶颈是**内存带宽**

**decode 是逐个 token 出的**，每出一个 token 都要把**激活参数从内存搬到计算单元**：

```
每秒能生成多少 token ≈ 内存带宽 ÷ 每 token 要搬的参数量

理论速度比 = 27B ÷ 3B ≈ 9×     ← 理想上限
实测兑现   = 2.4 ~ 2.8×          ← 见 §12.7
```

**本质：MoE 用内存换速度。** 把"每 token 读一遍所有参数"变成"只读一小撮"，代价是全部专家都得常驻内存。

### 12.5 为什么体积反而更大

**所有专家都必须常驻内存**——即使每 token 只用 8 个，剩下 56 个也不知道下一个 token 会不会轮到。

```
MoE 35B-A3B:  权重文件 22 GB（64 个专家全存）／ 每 token 只用到约 2 GB
DENSE 27B:    权重文件 18 GB（全部要存）    ／ 每 token 用到全部 18 GB
```

> **一句话**：MoE 是「**存储全部、读取少量**」，dense 是「**存储全部、读取全部**」。
...
| 维度 | 结论 |
|------|------|
| 总参数量更大 | 35B > 27B，**知识容量更大**（这正是 MoE 的原始动机：变大但不变慢） |
| 单参数效率更低 | 每个专家只见识被路由给它的数据，"每参数智能度"不如 dense |
| 实测 | 两道题打平；一般规律是**总参数相当时 MoE 略弱于 dense，但激活参数相当时 MoE 明显更强** |

> ⚠️ **别用激活参数比质量。** `A3B` 的 3B 是**推理成本**指标，**不是能力指标**。能力看总参数量。

**第二层 · 工程上（有实打实代价）**

| 代价 | 说明 | 对本地单人用有影响吗 |
|------|------|-------------------|
| 内存占用更高 | 约 22 GB vs 19 GB | 48GB 档上无感 |
| **冷启动更慢** | 5.2 vs 3.5 s（多读 3GB 磁盘） | ⚠️ 有，所以要调长 `keep_alive` |
| **并发效率会掉** | 多请求的 token 路由到不同专家 → 内存随机访问变多，**批处理吞吐可能不如 dense** | 单人无影响，做服务要注意 |
| **微调更麻烦** | 路由器不稳定，易出现"专家负载不均"（少数专家被过度使用） | 要微调时才知道 |
| **存在内存悬崖** | 超内存就完全不能用，没有"降级"选项 | 这就是 48G 的硬边界 |

**实测印证了内存悬崖**：即使开了 `OLLAMA_MAX_LOADED_MODELS=2`，仍受"系统分给 GPU 的可用内存上限"约束——

```
尝试同时常驻 27b（约 19 GB）+ 35b（约 22 GB）≈ 41 GB  >  系统允许的上限
实测结果：常驻 1 个，35b     ← Ollama 自己算完放不下，只留一个
```

### 12.7 为什么实测只有 2.4×，不到理论 9×

| 因素 | 影响 |
|------|------|
| **注意力层 + KV Cache** | 这部分 MoE 和 dense 一样，不因 MoE 变快 → 摊薄收益 |
| **权重每 token 重读** | 统一内存没有模型专用片上缓存，3B 参数每 token 都要再搬一次 |
| **路由器开销** | 门控计算 + top-k 选择 + 额外内存访问 |
| **专家内存不连续** | 8 个专家散落在 22GB 里 → **随机访问**，不如 dense 顺序读取 |
| **框架优化程度** | 稀疏计算在 MLX/Metal 上的优化不如稠密矩阵乘成熟 |

### 12.8 选型结论

```mermaid
flowchart TD
    Q[选 dense 还是 MoE?] --> A{内存够装吗?}
    A -->|不够| B[降档 dense 或换小模型]
    A -->|够| C{单人用还是高并发服务?}
    C -->|单人/本地| D["⭐ MoE<br/>2.4× 速度，质量同档"]
    C -->|高并发| E[dense 更稳<br/>MoE 批处理效率会掉]
    D --> F{要微调吗?}
    F -->|要| G["⚠️ MoE 路由不稳定<br/>优先 dense"]
    F -->|不要| H["✅ MoE 最优"]
...
> **一句话记法**：MoE = **多请几个专家，但每次只叫两个来**。花钱多（内存占得多），办事快（读得少），水平不差（总参数更大），但"专家之间协调"是有成本的。

**48GB Mac 上的结论：`35b-a3b` 是明确更优的选择。**
