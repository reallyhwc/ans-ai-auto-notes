...
> 关联: [贝叶斯统计与AI的关系](<../../计算机基础/贝叶斯统计与AI的关系.md>) — 概率视角理解 LLM 的 token 预测机制
...
**2025-2026 年的转向：稠密参数停滞、MoE 路线主导。** 单纯堆稠密参数的边际收益放缓，主流前沿模型（GPT-5.x、Claude Opus 4.x、Gemini 2.x/3、DeepSeek-V4、Llama 4）转向 **MoE（Mixture of Experts）架构**——总参数巨大、但每次推理只激活其中一小部分（DeepSeek-V4 已是 1M 上下文的 MoE 旗舰，DeepSeek-V3 时代的"总参数 671B / 激活 37B"仍是理解 MoE 的最好样本，见 [§7.4](#7-4-moe-mixture-of-experts-混合专家)）。同样能力下推理成本和访存量大幅降低，定价能压到 70B 稠密模型的 50% 左右。

> **模型迭代很快**，本文出现的具体版本号（GPT-5.x / Claude 4.x / DeepSeek-V4 / Llama 4）以 2026-09 为准，做选型时请以各厂商当前官方版本为准。
...
> 关联: [cnn](../基础/CNN（卷积神经网络）.md) — CNN 分层特征的涌现，与此处的"能力涌现"是同一类现象
...
| 收费 | 免费（内部实现细节） | 命中部分按原价的百分比收费：Anthropic ≈ **10%**（即 1 折）、OpenAI ≈ **50%**（即 5 折）、DeepSeek ≈ **10~50%**（按部位区分）—— 统一语义："命中价 / 原价" 越小越省 |
...
### 7.2 跑 70B 模型的实际推算（多卡或量化）

**先纠正一个常见错觉**：70B 模型 FP16 全精度需要 ≈ 140 GB 显存（每参数 2 byte），**单张 A100 80GB 装不下**。两条主流部署路径：

| 路径 | 显存占用 | 精度损失 | 适合场景 |
|------|---------|---------|---------|
| **2×A100 80GB FP16**（Tensor Parallel 张量并行拆分） | 每卡 ~70 GB 参数 + KV cache | 无 | 准确性敏感场景（评测、安全过滤） |
| **1×A100 80GB + INT8 量化** | ~70 GB | 通常 <1% | 大多数商用场景，本节按此口径算 |

下面以 **1×A100 80GB + INT8 量化** 为基准（参数从 140 GB 压到 70 GB，正好单卡能跑）：

```
A100 SXM 80GB 物理上限:
  ● 显存带宽 (HBM2e):         2,039 GB/s
  ● 算力 (INT8):              624 TOPS（FP16 312 TFLOPS 的 2 倍）
  ● 模型参数 (70B, INT8):      ~70 GB（剩余 ~10 GB 给 KV cache + activation）

Decode 阶段单步:
  ● 需要读取全部参数:          70 GB
  ● 理论单步耗时:              70 / 2,039 ≈ 34 ms → 29 steps/s
  ● 单请求:                    ~29 tokens/s（纯串行）

Continuous Batching 改变游戏（同时处理多个请求）:
  ● 处理 64 个请求，参数只读 1 次（所有人共享），KV cache 额外读 ~2 GB
  ● 单步: (70 + 2) / 2,039 ≈ 35 ms，但产出 64 个 token
  ● 实际 decode 吞吐:          29 × 64 ≈ 1,850 t/s
  ● Prefill 吞吐:              ~3,000 t/s（算力受限，但 batch 更大）

典型混合负载（prefill + decode 混合）:  ~2,000 总 t/s
```

### 7.3 单卡成本 vs 收入推演

```
每小时产出:  2,000 × 3,600 = 7,200,000 tokens/h
A100 租金:   ~$2.00/h（按时）/ ~$1.00/h（包年包月折合）
每百万 token 推理成本:  $2.00 / 7.2M × 1M ≈ $0.28/M

按 API 定价区分输入输出（参考 Llama 3.1 70B 在 DeepInfra/Together 等的市场价
                       假设输入:输出 = 3:1）:
┌──────────┬────────┬──────────┬──────────┐
│          │ 占比    │ 推理成本   │ API 定价  │
├──────────┼────────┼──────────┼──────────┤
│ 输入 token│ 75%    │ ~$0.12/M  │ $0.59/M   │
│ 输出 token│ 25%    │ ~$0.74/M  │ $0.79/M   │
└──────────┴────────┴──────────┴──────────┘

加权成本: 75% × $0.12 + 25% × $0.74 = $0.265/M token
加权定价: 75% × $0.59 + 25% × $0.79 = $0.640/M token

毛利率:    ($0.64 − $0.265) / $0.64 ≈ 58%
每小时毛利: ($0.64 − $0.265) × 7.2 = $2.70/h
理论月毛利: $2.70 × 730 ≈ $1,970/月（满负载，单卡）
实际 60% 负载: ~$1,180/月

→ 这是"理论最理想"上限，真实生产还要扣 NVLink 损耗、批次填不满的 padding、
  突发流量预留 buffer 等，实际利润通常再打 5~7 折。
→ 这也是为什么大家都在卷 INT4 量化和 MoE：稀疏激活 = 在大模型上拿小模型的吞吐。
```
...
> 下面用 DeepSeek-V3 当样本（数字公开、结构教科书式）；当前旗舰是 DeepSeek-V4，参数规模已上调、原生支持百万级上下文，"总参数 ≫ 激活参数"这个 MoE 特征不变。
...
```
DeepSeek V3 671B 总参数，单次推理只激活 37B（V4 已换代，但"总参数 ≫ 激活参数"的成本逻辑完全一致）
...
> 延伸: MoE 架构详解 → [本节 7.4](#7-4-moe-mixture-of-experts-混合专家)
> 延伸: 模型量化推理优化 → [本节 8](#8-模型量化-推理加速与显存压缩)
