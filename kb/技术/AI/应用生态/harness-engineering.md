...
> 关联: [外部参考链接](../../../实战/技巧/外部参考链接.md) — 系列文章链接汇总

---

## 团队级 Harness：从个人到团队的规模化实践

上面的讨论偏个人视角（一个人 + 一个 Agent），但真正有挑战的是**团队级别的 AI Coding 治理**。美团技术团队在 31 万行代码 AI 重构中的实践，是一个很好的团队级 Harness 案例。

### 核心方法："人人对齐 → 人机对齐"

```mermaid
flowchart LR
    A["人人对齐<br/>团队统一工程共识"] --> B["人机对齐<br/>共识固化为 AI Rule/Skill"]
    B --> C["AI 按统一规范产出"]
...
**为什么顺序不能颠倒？** 团队自己没有统一共识，AI Rule 写得再好也会被不同人解释成不同版本。人的共识是 AI 约束的前提。

一条关键原则：**1 个"独裁者"好过 10 个"民主者"**——需要一位强有力的角色拉齐所有成员的工程标准（分层原则、建模方式、依赖边界），再固化为 AI 可执行的约束。

### 团队级 Harness 组件对照

用美团案例对照六项核心能力在团队级别的体现：

| Harness 组件 | 个人级别 | 团队级别（美团实践） |
|-------------|---------|-------------------|
| **① 上下文设计** | CLAUDE.md | AI Rule（always 加载）：分层规范、业务域模型规约、仓储层规约，每次 AI 编码自动加载 |
| **② 工具设计** | build-index.js | Skill（渐进式加载）：Pre-PR 自查 Skill、迁移 SOP Skill，"编排类"与"能力类"职责边界 |
| **③ 约束与护栏** | "不要手改 overview.html" | 四层架构（Starter/Application/Infrastructure/Common）、依赖方向规则、PO 不泄露到上层 |
| **④ 反馈回路** | lint.sh + CI | Pre-PR 机制：AI 自查 3 轮（规范→Bug→一致性）→ AI 生成 PR 文档 → 人工聚焦业务语义 |
| **⑤ 诊断与调优** | 单次对话排查 | "专家经验定向 + AI 穷举扫描"：人圈定高危方向，AI 做全量扫描，发现 10 个隐藏性能隐患 |
| **⑥ 系统思维** | 一次定义，持续有效 | 主 R 打样 → SOP 分发 → 全组并行，规范随项目演进持续迭代 |

### Pre-PR：团队级反馈回路的关键实现

随着 AI 编码效率飙升，Code Review 成了全链路瓶颈——**"木桶效应"**：AI 压缩了编码时间，压力集中到 CR 环节，如果 CR 效率不提升，AI Coding 的提效红利会被 CR 瓶颈吞掉。

Pre-PR 的本质是**在反馈回路中增加一个 AI 前置过滤层**：

```
传统: 编码 → 人工 CR（60% 时间花在规范/命名/null check 上）
Pre-PR: 编码 → AI 自查 3 轮 → AI 生成 PR 文档 → 人工 CR（90% 时间聚焦业务语义）
```

### 团队级 Harness 的落地路径

```
Step 1: 盘清技术债 → 专家定向 + AI 穷举扫描
Step 2: 人人对齐   → 团队统一分层原则、建模方式、依赖边界
Step 3: 人机对齐   → 共识落地为 AI Rule（always）+ Skill（渐进式）
Step 4: 主R打样    → 一人跑通，沉淀 AI 可执行的迁移 SOP
Step 5: 建立Pre-PR → AI 编码提速后 CR 会成为新瓶颈，这一步不能省
```

> 关联: [AI Coding 团队治理](./ai-coding-team-governance.md) — 美团 31 万行代码重构完整记录（含详细的 5 步测试 SOP、零排期重构案例）
...
> 关联: [AI Coding 团队治理](./ai-coding-team-governance.md) — 美团31万行代码重构：人人对齐→人机对齐、Pre-PR、零排期重构
