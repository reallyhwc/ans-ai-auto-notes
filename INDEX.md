...
- [Hooks 事件全景与拦截机制](kb/技术/AI/Claude-Code/Hooks 事件全景与拦截机制.md) — Claude Code 全部 30 个 hook 事件按类别清单、阻断三档机制、配置层级合并、PreToolUse 完整示例、subagent/skill 内的 hook
...
- [本地部署 LLM](kb/技术/AI/大模型/本地部署 LLM.md) — Ollama安装使用+进阶玩法(API/Embedding/Modelfile/Web UI)、小模型推荐；§4 量化命名详解（Q4/Q5/Q6、_K_/S/M/L 含义与选档决策）；含 §8 Mac 微调可行性与成本（统一内存带宽对比、MPS 三个坑与芯片代际无关、云 GPU 租用 vs Mac 训练的时间账）、§9 Mac 上能玩的 14 个项目 + M4/M5 实测 tok/s 基准、§10 48GB 模型选型（Q4 内存公式与推荐清单）+ 闲置资源成本真相（内存昂贵、算力免费）与 keep_alive 调优、§11 本地跑通 Qwen3.6 27B/MoE 的真实记录（MLX 大幅超线性推算、两个硬编码/常驻坑）
...
- [Harness 与流程范式：SDD 落在哪一层](kb/技术/AI/应用/Harness 与流程范式：SDD 落在哪一层.md) — 区分 harness（结构轴·基建）与开发流程范式（时间轴·SOP），梳理 SDD/TDD/Goal-driven 等范式家族如何叠加在 harness 之上，对照 ans-ai-auto-notes 与 Raven ravenspec
...
- [MOSS-Music：开源音乐理解模型](kb/技术/AI/应用/MOSS-Music：开源音乐理解模型.md) — MOSS-Music = OpenMOSS 开源的 8B 音乐理解模型（音频编码器 + Qwen3-8B），DeepStack 跨层特征注入 + 时间标记让模型知道「第几秒发生了什么」，覆盖歌词 ASR / 和弦 / 曲式 / 长音频 QA；含扒源码核实的工程参数（12.5 token/s、2s 时间标记、无训练脚本）与情绪识别落地路径
...
- [沙箱（Sandbox）：从进程隔离到 Agent 运行时](kb/技术/计算机基础/沙箱（Sandbox）：从进程隔离到 Agent 运行时.md) — 把「沙箱」这个词拆成三种语境讲清楚：应用代码链路上的隔离环境（环境/流量/数据/凭证四维隔离 + 支付宝沙箱实例）、Agent 运行时的文件沙箱（DSH 的 workspace-write 与内核 runner）、OS 级隔离原语（chroot/namespace/seccomp/Landlock/Seatbelt）；附「这算不算真沙箱」的 5 问自检与常见误解
