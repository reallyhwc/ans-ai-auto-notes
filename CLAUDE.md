...
文件组织（同主题聚合到一文件、文件内时间倒序追加、新主题新建文件、目录深度）权威定义见 [kb-content-style skill](.claude/skills/kb-content-style/SKILL.md)（细则在 reference.md）。**中文文件名 = frontmatter title（强制性）**——命名与拆分细则见 reference.md。
...
> 操作细则（双输出原则、自检 checklist、控制台格式校验等）权威定义见 [kb-content-style skill](.claude/skills/kb-content-style/SKILL.md)；长文/URL 走 idea-extractor 的完整流程见 [.claude/agents/README.md](.claude/agents/README.md)。
...
1. **周记（人工叙事，按需写）**：`timeline/YYYY-WXX.md`，记录值得回看的对话摘要并链接到 kb 对应文件。**不是每周强制项**——做了一次较大沉淀（新增笔记、重构、重要修复）时写一条；无实质内容的周可以跳过，不会触发任何检查失败。（历史上 W21–W36 空档即按此口径处理，不回填。）
...
- 行数 >1000 只提示关注，**不擅自提案拆分**（拆分决策权归用户，除非用户主动要求）
- 章节编号必须从 1 连续无跳号
...
> 注：Stop hook 在 settings.json 里有两个 hook 数组（agent-log-hook.js main + exit-check.sh），两者都会执行。**Stop hook 在每次模型回复结束后触发**（不仅是用户退出时），所以自动 push 检查在每轮对话后都会运行。但 hook 输出只回传给模型，用户终端看不到——模型应在回复中主动报告 push 结果。
