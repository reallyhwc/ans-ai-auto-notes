...
`);
  ...
  // 原文件应有拆分提示（链接的 URL 部分空格被 encode 为 %20）
  assert.ok(orig.includes('已拆分'));
  assert.ok(orig.includes('章节%20A.md'), 'URL-encoded 文件名: ' + orig);
...
test('audit #3.1: 拆分提示链接的空格 URL-encode（CommonMark 严格解析兼容）', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'split-test-'));
  const big = path.join(dir, 'big.md');
  fs.writeFileSync(big, '# Big\n## 章节 A\n内容 A\n## 章节 B\n内容 B\n');
  splitDocBySections(big, ['章节 A']);
  const orig = fs.readFileSync(big, 'utf-8');
  // 拆分提示中的链接 URL 部分必须 URL-encode 空格（%20），CommonMark 严格解析才识别
  // ❌ 旧: > 已拆分到：[章节 A](./章节 A.md)   ← 空格未转义，部分 marked 配置不识别
  // ✅ 新: > 已拆分到：[章节 A](./章节%20A.md)
  assert.ok(orig.includes('[章节 A](./章节%20A.md)'),
    '拆分链接应 URL-encode 空格: ' + orig);
  assert.ok(!orig.match(/\(\.\/章节 A\.md\)/),
    '不应保留未转义空格: ' + orig);
  fs.rmSync(dir, { recursive: true, force: true });
});
